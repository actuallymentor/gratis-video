...
- Mediabunny is intentionally loaded with a dynamic import from the remux path so the main app bundle does not pay the parser/muxer cost at startup. Keep `can_attempt_remux_export()` cheap and metadata-only.
