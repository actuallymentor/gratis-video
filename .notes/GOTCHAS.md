...
- Recording capture constraints must not request width, height, aspect ratio, or post-open max dimensions. Mobile browsers can satisfy size requests by switching into cropped sensor modes; ask for the preferred camera and `resizeMode: none`, then use the browser-provided frame.
