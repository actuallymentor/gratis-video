...
- Native file share `File.type` should use bare shareable video MIME types such as `video/mp4` or `video/webm`; codec parameters from MediaRecorder MIME strings can make `navigator.canShare({ files })` reject the file.
- If a cached export is discovered during the export tap rather than preloaded, load its blob and route through the ready panel. Do not call `navigator.share()` after async cache work because transient activation may already be gone.
...
- To push Chrome mobile beyond its default camera mode without forcing 16:9 or square capture, open the camera without width/height/aspect ratio, read the active track orientation, then apply ideal preset dimensions and frame rate with `resizeMode: none` on that track.
- `MediaTrackCapabilities.width.max` and `height.max` are the maximum each dimension can take across any sensor mode and may not be simultaneously achievable. Do not use independent capability maxes as a recording target; match preset dimensions to `track.getSettings()` orientation before calling `applyConstraints`.
- Recording now reuses the live preview's video track and requests microphone audio separately. Do not clear/reopen the camera on record start unless there is no preview stream; that can switch lenses on multi-camera phones and make the saved clip disagree with the preview.
- Capture and export recorders intentionally call `MediaRecorder.start()` without a timeslice. Adding short timeslices increases main-thread/dataavailable churn and should be justified with a browser-specific reason.
- Export canvas playback should wait on `video.requestVideoFrameCallback()` when available, with only a fallback for browsers that do not support it. `requestAnimationFrame()` can run when video decode has not produced a new frame.
- The last camera choice is stored in global settings as `last_video_device_id`. Stale device ids must fall back to the browser default and update/clear the saved choice so a removed camera does not brick preview startup.
...
- Lossless export now attempts a Mediabunny remux before the canvas exporter, but only for source-resolution clips whose stored MIME metadata identifies one shared MP4/WebM container. Any codec, track, dimension, audio-presence, selected-format, or packet-read mismatch must fall back to the canvas exporter rather than failing the export.
...
- Recording audio mode is a persisted capture setting. `noise_cancelling` requests echo cancellation, noise suppression, and auto gain; `unfiltered` requests all three off, but mobile browsers may still partially ignore audio-processing constraints.
