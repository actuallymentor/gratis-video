...
- Recording capture constraints should not prefer a fixed 16:9 resolution like 1280x720. Mobile browsers can satisfy that by cropping the camera sensor, making clips look zoomed compared with the native camera app; prefer native `resizeMode: none` and the largest available track dimensions.
